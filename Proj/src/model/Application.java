...
AUTRE};
	...
	public Application(String fichierXml) {
		...
		pour_show_point = new ShortestPath();
		pour_show_point.init(reseau_routier, ZOOM_INITIAL);

		arePrinted = false;

		chemin = new Vector<ItineraryState>();
		
		// Initialisations des differents elements
	...
	public void showPoint() {
		if (!arePrinted){
			clearPoints(); // Effacer les points précédents
			List<Integer> idsPoints = pour_show_point.chercheTousLesPoints();
			for (int i = 0; i < idsPoints.size(); i++) {
				System.out.println(idsPoints.get(i));
				Point pointToPrint = pour_show_point.getNodeCoords(idsPoints.get(i));
				dessinerPoint(pointToPrint, Color.RED, 10);
			}
			arePrinted = true;
		} else {
			clearPoints();
	...
	public void changeMap(){
        ...
        String dataDirectory = repertoireCourant + "/data";

        // Création du JFileChooser avec le répertoire de données
        JFileChooser dialogue = new JFileChooser(dataDirectory);

        // Définition du filtre pour les fichiers XML
        FileFilter xmlFilter = new FileFilter() {
            @Override
            public boolean accept(File file) {
                // Accepter les répertoires pour permettre la navigation
                if (file.isDirectory()) {
                    return true;
                }
                // Accepter les fichiers avec l'extension .xml
        ...
        // Appliquer le filtre au JFileChooser
        ...
        // Si un fichier a été sélectionné, créer une nouvelle instance de l'application
        if (returnValue == JFileChooser.APPROVE_OPTION) {
            new Application(dialogue.getSelectedFile().toString());
        }
