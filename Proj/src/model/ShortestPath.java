...
public class ShortestPath {
	...
	public List<Integer> chercheTousLesPoints() {
    	// Recherche tous les points et retourne leurs identifiants
    	Point pointCourant;
    	Node node;
    	int i;
    	List<Integer> idsPoints = new ArrayList<>();
		
    	for (i = 0; i < n; i++) {
    	    node = v[i];
    	    pointCourant = new Point(node.x, node.y);
    	    idsPoints.add(node.id);
    	}
